...
class ProductSpecifications:
    ...
    def generate_filter_values(self,product_description,filter_key):
        prompt="Only Generate values for {items} corresponding to the previously generated keys. The keys we generated are: {keys}. Please ensure that each key and value are on separate lines.don't give number to each line.and don't give any other key instead {keys}and key value pair should be in this format(key:pair)"
        prompt_template=ChatPromptTemplate.from_template(prompt)
